...
import static com.ctriposs.sdb.DbBenchmark.DBState.EXISTING;
import static com.ctriposs.sdb.DbBenchmark.DBState.FRESH;
import static com.ctriposs.sdb.DbBenchmark.Order.RANDOM;
import static com.ctriposs.sdb.DbBenchmark.Order.SEQUENTIAL;
...
public class DbBenchmark {
	...
	private static String databaseDir = TestUtil.TEST_BASE_DIR + "sdb/unit/benchmark";
            ...
            throws IOException
    {
        ...
        for (String benchmark : benchmarks) {
            ...
            if (benchmark.equals("fillseq")) {
                write( SEQUENTIAL, FRESH, num_, valueSize, 1);
            }
            else if (benchmark.equals("fillbatch")) {
                write(SEQUENTIAL, FRESH, num_, valueSize, 1000);
            }
            else if (benchmark.equals("fillrandom")) {
                write(RANDOM, FRESH, num_, valueSize, 1);
            }
            else if (benchmark.equals("overwrite")) {
                write(RANDOM, EXISTING, num_, valueSize, 1);
            }
            else if (benchmark.equals("fillsync")) {
                write(RANDOM, FRESH, num_ / 1000, valueSize, 1);
            }
...
//        }
    ...
    private void write(Order order, DBState state, int numEntries, int valueSize, int entries_per_batch)
            throws IOException
    {
        ...
        for (int i = 0; i < numEntries; i += entries_per_batch) {
            ...
            for (int j = 0; j < entries_per_batch; j++) {
                int k = (order == Order.SEQUENTIAL) ? i + j : rand_.nextInt(num_);
            ...
            for(Entry<byte[], byte[]> entry : batch) {
            	sdb_.put(entry.getKey(), entry.getValue());
            }
        }
    }
